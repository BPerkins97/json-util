...
public class Json {
    ...
    public static JsonNode parse(String json) {
        ...
        int pointer = 0;
        while (pointer < numCodePoints && isWhitespace(json.codePointAt(pointer))) {
            pointer++;
        }
        ...
        for (int i=pointer+1;i<numCodePoints;i++) {
                    ...
                    if (!isWhitespace(codePoint)) {
                        switch (previousMode) {
                            case MODE_OBJECT_START -> {
                                if (codePoint != '"') {
                                    throw new IllegalArgumentException("A key has to be quoted.");
                                }
                            ...
                            case MODE_KEY -> {
                                if (codePoint != ':') {
                                    throw new IllegalArgumentException("A key definition has to be followed by a colon.");
                                }
                            ...
                            case MODE_VALUE -> {
                                // TODO throw exception when is not quote
                                previousMode = MODE_WHITESPACE_CONSUME;
                                mode = MODE_KEY;
                            }
                        }
